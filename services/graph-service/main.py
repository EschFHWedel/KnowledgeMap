...
def get_graph(
    ...
    with driver.session() as session:
        if rootId is None:
            ...
            # list(...) materialisiert die Records innerhalb der Session;
            # rec["n"] ist ein Node-, rec["r"] ein Relationship-Objekt.
            node_recs = list(session.run("MATCH (n:Node) RETURN n"))
            edge_recs = list(session.run(
                """
